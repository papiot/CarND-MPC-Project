...
## Project Rubric reflections

The project implementation is using Kinematic Models, which are simplifications of dynamic models which ignore tire forces, gravity and mass.
The simplification will reduce accuracy, but it will make them more tracable which is the purpose of this assignment. (at moderate speeds)

The model has the follwing states and actuators:
States:
x(x_position)
y(y_position)
psi(orientation)
...
The update equations are implemented as follows:
```
...
## Timestep Length and Elapsed Duration parameters
The final timestep was:
N=11
and dt=0.05s

Setting the N to a high number (such as 30), the car went off the track.
Setting the N to a small number (such as 3), the car started oscillating and did not drive in a straight line
With fixed N and large dt, the car oscillates too much
